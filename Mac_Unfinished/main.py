...
def default_recurrence_range_end(rule: str, start_dt: datetime) -> datetime:
    ...
    if rule == "monthly":
        return safe_add_months_preserve_day(base, 24)
    if rule == "yearly":
        return safe_replace_year(base, base.year + 10)
    return base
...
class PlanDialog(GlassDialog):
    ...
    def __init__(self, parent, start_dt: datetime, end_dt: datetime, categories, preset_cat=None, preset_task=None, title="Plan Task", preset_recurrence: str = "none", preset_all_day: bool = False, preset_recurrence_range_start: str = "", preset_recurrence_range_end: str = "", allow_delete: bool = False, delete_label: str = "Delete"):
        ...
        try:
            ...
            if preset_recurrence and str(preset_recurrence).strip().lower() != 'none':
                preset_range_end_dt = default_recurrence_range_end(preset_recurrence, preset_range_start_dt)
            else:
                preset_range_end_dt = start_of_day(end_dt)

        self._updating_recurrence_range = False
        self._auto_recurrence_range_end = not str(preset_recurrence_range_end or '').strip()
        ...
        self.recur_start_date_edit.dateChanged.connect(self._handle_recurrence_start_changed)
        self.recur_end_date_edit.dateChanged.connect(self._mark_recurrence_range_end_manual)
    ...
    def _set_recurrence_end_date(self, dt: datetime):
        self._updating_recurrence_range = True
        try:
            self.recur_end_date_edit.setDate(QDate(dt.year, dt.month, dt.day))
        finally:
            self._updating_recurrence_range = False

    def _mark_recurrence_range_end_manual(self, *_):
        if not getattr(self, "_updating_recurrence_range", False):
            self._auto_recurrence_range_end = False

    def _handle_recurrence_start_changed(self, *_):
        if self.recurrence() != 'none' and getattr(self, "_auto_recurrence_range_end", False):
            self._set_recurrence_end_date(default_recurrence_range_end(self.recurrence(), self.recurrence_range_start_date()))
    ...
    def _sync_recurrence_range_state(self):
        ...
        if enabled and getattr(self, "_auto_recurrence_range_end", False):
            self._set_recurrence_end_date(default_recurrence_range_end(self.recurrence(), self.recurrence_range_start_date()))
